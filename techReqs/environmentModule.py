...
import subprocessimport NXOpen
import NXOpen.BlockStyler
import subprocess
import re
...
class ColoredBlock:
	...
	def __init__(self):
		...
		try:
			...
			self.theUI = NXOpen.UI.GetUI()
			self.theSessionName = self.theSession.ExecutingJournal #путь до файла 
			self.theDialogName =  '%s.dlx' %self.theSession.ExecutingJournal.split('.')[0]# путь в dlx
			self.theDialog = self.theUI.CreateDialog(self.theDialogName)
			self.theDialog.AddApplyHandler(self.apply_cb)
			self.theDialog.AddInitializeHandler(self.initialize_cb)
			self.theDialog.AddDialogShownHandler(self.dialogShown_cb)
	...
	def apply_cb(self):
		...
		try:
			...
			prop = self.enum0.GetProperties().GetEnum("Value")
			if prop == 0:
				self.createTechRequirements()
			elif prop == 1:
				pass #self.rebuildTechRequirements(text)
			elif prop == 2:
				pass #self.removeTechRequirements(self)
			else:
				pass
	...
	def createTechRequirements():
		pass	
	
	def rebuildTechRequirements():
		pass

	def removeTechRquirements(self):
		pass	
###		
def updateSwitcher(theSession,theUI,switchMode):
	workPart = theSession.Parts.Work
	displayPart = theSession.Parts.Display
	lw = theSession.ListingWindow
	resUpdate = 'включено' if switchMode else 'выключено'
